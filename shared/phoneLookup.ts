...
// ODIS quota top-up guardrails: 0.05 cUSD buys ~50 lookups; refill only when
// exhausted, never more than once per process run.
const TOPUP_CUSD_WEI = "50000000000000000"; // 0.05 cUSD
let toppedUpThisProcess = false;
...
async function ensureQuota(
  ...
  if (toppedUpThisProcess) throw new Error("ODIS quota exhausted and already topped up this run");
  toppedUpThisProcess = true;
  log.info({ account, remaining }, "ODIS quota exhausted; paying 0.05 cUSD for more");
